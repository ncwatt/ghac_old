...
class AREOI_Styles
{
	...
	public static function add_block_styles()
	{
		...
		foreach ( $devices as $device_key => $device ) {
			...
			$inner_styles = self::add_block_style( $blocks, $device_key, $is_cssgrid, $display_units );
	...
	public static function add_block_style( $blocks, $device_key, $is_cssgrid, $display_units )
	{	
		$styles = '';
		if ( !empty( $blocks ) ) {
			foreach ( $blocks as $block_key => $block ) {
			    ...
			    if ( !empty( $block['innerBlocks'] ) ) {
			    	$styles .= self::add_block_style( $block['innerBlocks'], $device_key, $is_cssgrid, $display_units );
			    }
			}
		}
