...
function areoi_get_class_name_str( $classes )
{
	...
	return esc_attr( trim( $class_string ) );
}
...
{
	return ( ( !empty( $attributes['anchor'] ) ) ? 'id="' . esc_attr( $attributes['anchor'] ) . '"' : '' );
}
...
{
	return esc_attr( 'block-' . $block_id );
}

function areoi_get_rgba_str( $rgba )
{
	return esc_attr( trim( 'rgba(' . $rgba['r'] . ', ' . $rgba['g'] . ', ' . $rgba['b'] . ',' . $rgba['a'] . ')' ) );
}
